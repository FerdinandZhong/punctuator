...
from .dataset import read_data, PunctuatorDataset
...
class ValidationArguments(BaseModel):
    ...
    Args:
        ...
        model_name(str): name or path of fine-tuned model
        ...
        validated_puncs(List[str]): list of puncs to be validated
...
class ValidationPipeline:
    def __init__(self, validation_arguments) -> None:
        ...
        if validation_arguments.tag2id_storage_path:
            ...
            self.tag2id = self.classifer.config.label2id
    ...
    def validate(self):
        ...
        with tqdm(total=len(val_loader)) as pbar:
            for batch in val_loader:
                ...
                true_preds, true_labels = self._post_process(logits, labels, attention_mask)
                self.total_preds.extend(true_preds)
                self.total_labels.extend(true_labels)
        ...
        tested_labels = []
        target_names = []
        for tag, id in self.tag2id:
            tested_labels.append(id)
            target_names.append(tag)
        report = classification_report(total_labels, total_preds, labels=tested_labels, target_names=target_names)
