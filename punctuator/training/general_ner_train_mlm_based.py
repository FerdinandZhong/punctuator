...
class EncodingDataset:
    def __init__(self, encodings, labels, token_types):
    ...
    def __getitem__(self, idx):
        ...
        item["token_types"] = torch.tensor(self.token_types[idx])
        return item
...
class NERTrainingArguments(BaseModel):
    ...
    def generate_corpus(args: argparse.Namespace):
        ...
        (training_corpus, training_tags,) = read_data_for_mlm(
            training_raw,
        ...
        (validation_corpus, validation_tags,) = read_data_for_mlm(
            val_raw,
            args.min_sequence_length,
            args.max_sequence_length,
            is_split_into_words=args.is_split_into_words,
...
class NERTrainingPipeline:
    def __init__(self, training_arguments):
        ...
        self.train_encoded_token_types = None
        self.validation_encoded_token_types = None
    ...
    def tokenize(self):
        ...
        self.train_encoded_tags, self.train_encoded_token_types = self._encode_tags(
            self.arguments.training_tags,
            self.train_encodings,
        )
        self.validation_encoded_tags, self.validation_encoded_token_types = self._encode_tags(
            self.arguments.validation_tags,
            self.val_encodings,
        )
    ...
    def _accuracy(self, logits, token_type_ids, labels):
        if self.device.type == "cuda":
            max_preds = logits.argmax(dim=-1).detach().cpu().numpy().flatten()
            flattened_labels = labels.detach().cpu().numpy().flatten()
            token_type_ids = token_type_ids.detach().cpu().numpy().flatten()
